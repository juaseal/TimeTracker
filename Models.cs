...
public sealed class SummaryRow : INotifyPropertyChanged
{
    ...
    public string DayText=>Day.ToString("ddd dd/MM",CultureInfo.GetCultureInfo("en-US"));
...
public sealed class WeekDayTotal { public DateTime Day { get; set; } public TimeSpan Total { get; set; } public double ExpectedHours { get; set; } public string DayText=>Day.ToString("ddd dd/MM",CultureInfo.GetCultureInfo("en-US")); public string TotalText=>SessionRow.Format(Total); public string ExpectedText=>SessionRow.Format(TimeSpan.FromHours(ExpectedHours)); public double DifferenceHours=>Total.TotalHours-ExpectedHours; public string StatusText=>ExpectedHours<=0?"Off":DifferenceHours switch { > 0.01=>$"+{SessionRow.Format(TimeSpan.FromHours(DifferenceHours))}", < -0.01=>$"-{SessionRow.Format(TimeSpan.FromHours(-DifferenceHours))}", _=>"OK"}; public Brush StatusBrush=>ExpectedHours<=0?Brushes.Gray:DifferenceHours>0.01?Brushes.Firebrick:DifferenceHours< -0.01?Brushes.DarkOrange:Brushes.ForestGreen; }
